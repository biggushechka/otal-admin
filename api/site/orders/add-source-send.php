...
$source = $POST['source'];
$email = $POST['email'];
$tg_token = $POST['tg_token'];
$tg_chad_id = $POST['tg_chad_id'];
$currentDateTime = date('Y-m-d H:i:s');

if ($method === "POST") {

    if ($source === "email") {
        $query_add_row = $dbh->prepare("INSERT INTO `site_orders_source_send` SET
            `id_site` = :id_site,
            `email` = :email,
        ...
        if ($query_add_row->rowCount() > 0) {
            ...
            $query_get_meta = $dbh->prepare("SELECT `id`, `email` FROM `site_orders_source_send` WHERE `id` = :id");
