...
if ($method === "GET") {
    ...
    if ($source === "email") {
        $query_get_row = $dbh->prepare("SELECT * FROM `site_orders_source_email` WHERE `id_site` = :id_site");
        ...
        $query_get_row = $dbh->prepare("SELECT * FROM `site_orders_source_telegram` WHERE `id_site` = :id_site");
        $query_get_row->execute(["id_site" => $id_site]);
