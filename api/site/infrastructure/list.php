...
// Получение
if ($method === "GET") {
    $query_get_row = $dbh->prepare("SELECT * FROM `site_infrastructure` WHERE `id_site` = :id_site");
...
if ($method === "POST") {
    ...
    foreach ($webpImages as &$image) {
        ...
        if ($saveImg != "false") {
            ...
            // добавляем запись в таблицу "site_infrastructure"
            $query_add = $dbh->prepare("INSERT INTO `site_infrastructure` SET
                `id_site` = :id_site,
                `photo` = :photo,
                `title` = :title,
            ...
            if ($query_add->rowCount() != 0) {
                ...
                $query_get_new_row = $dbh->prepare("SELECT * FROM `site_infrastructure` WHERE id = :id");
...
function updateRow() {
    ...
    $query = $dbh->prepare("UPDATE `site_infrastructure` SET  
        `title` = :title, 
        `description` = :description, 
        `date_create` = :date_create 
    WHERE `id` = :id");
    ...
    if ($photos != "") {
        ...
        $query_get_row = $dbh->prepare("SELECT * FROM `site_infrastructure` WHERE id = :id");
        ...
        foreach ($webpImages as &$image) {
            ...
            if ($saveImg != "false") {
                ...
                // добавляем запись в таблицу "site_infrastructure"
                $query_uppdate_row = $dbh->prepare("UPDATE `site_infrastructure` SET `photo` = :photo WHERE `id` = :id");
                $query_uppdate_row->execute(["id" => $id, "photo" => $filePath]);

                if ($query_uppdate_row->rowCount() > 0) {
                    $query_get_update_row = $dbh->prepare("SELECT * FROM `site_infrastructure` WHERE id = :id");
...
if ($method === "DELETE") {
    ...
    $query_delete_adv = $dbh->prepare("DELETE FROM `site_infrastructure` WHERE `id` = :id");
