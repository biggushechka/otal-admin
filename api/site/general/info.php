...
if ($method === "POST") {
    ...
    $query_findSiteID = $dbh->prepare("SELECT `id` FROM `my_sites` WHERE `domain` = :domain LIMIT 1");
    $query_findSiteID->execute(["domain" => $domain]);
    $siteID = $query_findSiteID->fetchColumn();

    if ($siteID !== false) {
        $query_update_general = $dbh->prepare("UPDATE `project_general` SET `title_project` = :title_project, `phone` = :phone, `email` = :email, `telegram_phone` = :telegram_phone, `telegram_link` = :telegram_link, `whatsapp_phone` = :whatsapp_phone, `whatsapp_link` = :whatsapp_link WHERE `id_site` = :id_site");
        $query_update_general->execute(["id_site" => $siteID, "title_project" => $title_project, "phone" => $phone, "email" => $email, "telegram_phone" => $telegram_phone, "telegram_link" => $telegram_link, "whatsapp_phone" => $whatsapp_phone, "whatsapp_link" => $whatsapp_link]);
